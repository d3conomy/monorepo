...
describe('src/log-books-manager/LogBooksManager.js', () => {
    ...
    it('should get all entries from all log books', () => {
        ...
        const entry1 = new LogEntry({ message: 'Test message 1', level: LogLevel.INFO });
        const entry2 = new LogEntry({ message: 'Test message 2', level: LogLevel.DEBUG });
        logBook1.add(entry1);
        logBook2.add(entry2);
        const allEntries = logBooksManager.getLastEntries();
        expect(allEntries.size).to.be.equal(2);
        expect(allEntries.get(1)).to.be.equal(entry1);
        expect(allEntries.get(2)).to.be.equal(entry2);
    });
});
