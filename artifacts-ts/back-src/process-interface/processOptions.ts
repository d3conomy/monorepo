...
type ProcessOptionsParams = { [key: string]: any }
...
class ProcessOptions extends Map<ProcessOption['name'], IProcessOption>{
    ...
    static fromParams(params: { [key: string]: any }): ProcessOptions {
        let options = new ProcessOptions([]);
        for (const [key, value] of Object.entries(params)) {
            options.set(key, createProcessOption({ name: key, value: value }));
        }
        return options;
    }

    findOption(name: string): IProcessOption | undefined {
        return this.get(name);
    }

    toParams(): ProcessOptionsParams {
    ...
    toArray(): Array<IProcessOption> {
        // return the values not the functions
